...
var PThread = {
  ...
  freeThreadData: function (pthread) {
    ...
    if (pthread.threadInfoStruct) {
      var tlsMemory =
        GROWABLE_HEAP_I32()[(pthread.threadInfoStruct + 104) >> 2];
...
function _fd_close(fd) {
  if (ENVIRONMENT_IS_PTHREAD)
    return _emscripten_proxy_to_main_thread_js(1, 1, fd);
  return 0;
}
...
function _fd_write(fd, iov, iovcnt, pnum) {
  if (ENVIRONMENT_IS_PTHREAD)
    return _emscripten_proxy_to_main_thread_js(3, 1, fd, iov, iovcnt, pnum);
  var num = 0;
...
var JSEvents = {
  ...
  queueEventHandlerOnThread_iiii: function (
    ...
    _emscripten_async_queue_on_thread_(
      targetThread,
      637534208,
      eventHandlerFunc,
      eventData,
...
function _emscripten_webgl_do_create_context(target, attributes) {
  ...
  contextAttributes.proxyContextToMainThread =
    GROWABLE_HEAP_I32()[a + (48 >> 2)];
  contextAttributes.renderViaOffscreenBackBuffer =
    GROWABLE_HEAP_I32()[a + (52 >> 2)];
  var canvas = __findCanvasEventTarget(target);
  if (!canvas) {
    return 0;
  }
...
var _emscripten_async_queue_call_on_thread = (Module[
  "_emscripten_async_queue_call_on_thread"
] = function () {
  return Module["asm"]["ca"].apply(null, arguments);
});
var _emscripten_async_queue_on_thread_ = (Module[
  "_emscripten_async_queue_on_thread_"
] = function () {
...
var asmJsEstablishStackFrame = (Module["asmJsEstablishStackFrame"] =
  function () {
    return Module["asm"]["Na"].apply(null, arguments);
  });
